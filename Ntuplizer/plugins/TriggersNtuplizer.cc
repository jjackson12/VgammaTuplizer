...
<<<<<<< HEAD
   if( 
       ...
       //MERGE NOTE: Above is John's Code. Below is di-boson code. I'm including both (checks on both lists of variables). The other possibility seems to include again only the photon checks, seeing as that is what John did for his.

       trigName.find("AK8PFJet360_TrimMass30") != std::string::npos ||
  ...
  if (doTriggerObjects_) {
	...
	for (unsigned hh = 0; hh < obj.filterLabels().size(); ++hh){
	  if(findFilter(obj.filterLabels()[hh])){
        ...
        //======= MERGE NOTE: John's code above, dijet below. Keeping both.
